    ...
    >
      ...
      >
        <transition-group
            ...
            class="list-main"
            :class="[
						data.config.columns === 'two' ? 'flexColumns' : '',
					]"
        >
          <layoutItem
              class="drag-move"
    ...
    >
      <ul>
        <li @click="handleDownMerge">
          <el-icon type="caret-down" />向下合并
        </li>
        <li @click="handleRightMerge">
          <el-icon type="caret-right" />向右合并
        </li>
        <li @click="handleAddCol">
          <el-icon type="border-horizontal" />增加一列
        </li>
        <li @click="handleAddRow">
          <el-icon type="border-verticle" />增加一行
        </li>
      </ul>
    </div>
  </div>
  ...
  methods: {
      ...
      const traverse = (array) => {
        array.forEach((element, index) => {
          ...
          if (element.type === "grid" || element.type === "tabs") {
            ...
            if (
                !isCopy &&
                !this.insertAllowedType.includes(data.type)
            ) {
    ...
    handleDownMerge() {
      ...
      if (
          this.rightMenuSelectValue.trs[this.trIndex].tds[this.tdIndex]
              .colspan !==
          this.rightMenuSelectValue.trs[this.trIndex + currentRowspan]
              .tds[this.tdIndex].colspan
      ) {
        this.$message.error("当前表格无法向下合并");
        return false;
      }
